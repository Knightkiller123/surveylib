...
public class SomeoneElseDetailAdapter    extends RecyclerView.Adapter<SomeoneElseDetailAdapter.ViewHolder> {
    private Context context;
    private List<Family_profile> list;
    private Prefs mPrefs;
    private SomeoneElseDetailAdapter.OnUpdateData onUpdateData;
    public SomeoneElseDetailAdapter(Context context, List<Family_profile> family,SomeoneElseDetailAdapter.OnUpdateData onUpdateData) {
    ...
    @Override
    public SomeoneElseDetailAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater layoutInflater=LayoutInflater.from(parent.getContext());
        View view=layoutInflater.inflate(R.layout.content_family_detail,parent,false);
        return new SomeoneElseDetailAdapter.ViewHolder(view);

    }

    @Override
    public void onBindViewHolder(SomeoneElseDetailAdapter.ViewHolder holder, final int position) {
       ...
       /* if(holder.tvTag!=null)
        ...
        holder.tvKnowTheirSchemes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mPrefs.save(Constants.PositionRestart,position);
    ...
    public void restart()
    {
        int position = mPrefs.getInt(Constants.PositionRestart,0);
        mPrefs.save(Constants.FromSomeoneElseGo,"yes");
        mPrefs.save(Constants.ProfileId,list.get(position).id);
        mPrefs.save(Constants.FirstName,list.get(position).name);
        onUpdateData.dismissDialogg();
